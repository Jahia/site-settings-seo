import {publishAndWaitJobEnding, createSite, deleteSite, deleteNode, getVanityUrl, getNodeByPath} from '@jahia/cypress'
...
describe("Basic tests of the module's seo filter", () => {
    ...
    it('Add a first basic vanity URL from the UI', function () {
        ...
        let contextMenu = composer.openContextualMenuOnLeftTree(pageVanityUrl1)
        ...
        getNodeByPath(homePath+ "/"+pageVanityUrl1 +"/vanityUrlMapping/vanity1", ["j:default"], "en", [], "LIVE").then((result) => {
            expect(result?.data).eq(undefined)
        })
        // Check it is not canonical by default
        getNodeByPath(homePath+ "/"+pageVanityUrl1 +"/vanityUrlMapping/vanity1", ["j:default"], "en", [], "EDIT").then((result) => {
    ...
    it('Add a first canonical and published vanity URL from the UI', function () {
             ...
             const composer = new CustomPageComposer()
             CustomPageComposer.visit('digitall', 'en', 'home.html')
             let contextMenu = composer.openContextualMenuOnLeftTree(pageVanityUrl2)
             const contentEditor = contextMenu.edit()
             const vanityUrlUi = contentEditor.openVanityUrlUi()
        ...
        cy.wait(500)
             ...
             // Check the vanity url is not published by default
            getNodeByPath(homePath+ "/"+pageVanityUrl2 +"/vanityUrlMapping/vanity2", ["j:default"], "en", [], "LIVE").then((result) => {
                 expect(result?.data).eq(undefined)
             })
             // Check it is canonical
        vanityUrlUi.checkVanityUrlDefaultBadge("/vanity2")
             getNodeByPath(homePath+ "/"+pageVanityUrl2 +"/vanityUrlMapping/vanity2", ["j:default"], "en", [], "EDIT").then((result) => {
                 expect(result?.data).not.eq(undefined)
